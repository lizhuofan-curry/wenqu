...
- 响应式洞察布局：三项指标在桌面端等宽铺满，中等宽度下第三项自动占满余下整行；保持率摘要、笔记导出与筛选在 980px 以下提前改单列，搜索与材料筛选不会挤压换行；诊断路线与证据笔记在 320–390px、明暗主题下支持长中英文和无空格标识符断行，并保持现有宣纸、墨色与朱砂视觉体系；
...
- 本地多材料专题：把多份材料归入自定义专题分组、按专题筛选资料库，专题卡片直达各材料的陪读与诊断入口；分组仅保存在当前浏览器、不跨设备同步，跨材料 AI 知识地图留待云端版本；
...
- [证据笔记云端同步安全设计（草案，实施待授权）](docs/design/evidence-notes-cloud-sync.md)
...
- **后续学习效果路线**：真实课前/课后异题验证、语音复述；证据笔记云端同步已完成[独立安全设计文档](docs/design/evidence-notes-cloud-sync.md)，实施需单独授权生产迁移；多材料专题已落地本地 MVP（分组仅存当前浏览器），云端化随迁移一并实施。
