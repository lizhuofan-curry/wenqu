...
发布顺序必须是：先完成云同步恢复中心的服务端独占写迁移与发布，再应用 `202608250002_transfer_tasks.sql`，验证私有表/RPC/权限，最后部署迁移题应用。代码合入不等于功能已上线。
...
- **v.5 开发中**：云同步恢复中心、服务端独占学习档案写入、错因驱动迁移检验、隐私合规、监控与首轮 5 人封闭测试；
- **后续学习效果路线**：延迟保持率、课前诊断、自适应路线、证据笔记卡、语音复述与多材料专题。
...
问渠 v.4 已完成“材料 → 学习 → 评分 → 云端档案 → D1/D3/D7 复习”的生产闭环验收。v.5 的云同步恢复中心与错因驱动迁移检验已进入分阶段发布门禁；合入代码不等于已经应用生产数据库迁移或完成 Production 发布。
