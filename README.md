...
- **v.5 开发中**：云同步恢复中心、服务端独占学习档案写入、隐私合规、监控与首轮 5 人封闭测试；
- **后续学习效果路线**：错因驱动迁移题、延迟保持率、课前诊断、自适应路线、证据笔记卡、语音复述与多材料专题。
...
问渠 v.4 已完成“材料 → 学习 → 评分 → 云端档案 → D1/D3/D7 复习”的生产闭环验收。v.5 正在加入可验证、不可篡改且不重复计费的云同步失败恢复；合入代码不等于已经应用生产数据库迁移或完成 Production 发布。
