...
> 2026-08-24 状态更新：下述审计结论仍准确描述当时的 `main`/生产版本；对应 P0/P1 修复已通过提交 `5f2c39f` 推送到 GitHub `codex/security-hardening` 分支，但尚未应用 Supabase 迁移、合并 `main` 或部署，因此生产安全状态尚未改变。
...
8. 本地学习记录按 user id 分区，移除登录自动迁移；认证世代使账号切换前的材料、档案、学习、上传、评分、删除和重新生成请求晚返回时失效，云端记录保存还校验请求发起时的 owner；同步失败与本机兜底在界面明确展示。
...
- 代码已推送安全分支，但尚未与分叉的最新 `origin/main` 对齐、合并、部署或在公网执行 A/B 账号隔离闭环；当前生产站仍应按原审计结论看待。
