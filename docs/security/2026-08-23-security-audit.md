...
> 2026-08-24 状态更新：下述审计结论仍准确描述当时的 `main`/生产版本；对应 P0/P1 修复已在本地 `codex/security-hardening` 分支实现并通过回归，但尚未应用 Supabase 迁移、部署或推送，因此生产安全状态尚未改变。
...
- 代码尚未部署、推送或在公网执行 A/B 账号隔离闭环；当前生产站仍应按原审计结论看待。
