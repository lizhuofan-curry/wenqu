...
- 原本地 `main` 相对远端领先 8、落后 80，未强推或改写；已保留为 `backup/main-pre-sync-20260826`，随后从 `origin/main` 重建本地 `main`。
- `codex/evidence-note-cards` 上两份未提交进展记录先保存为可恢复 stash，再以最新 `main` 为基线人工合并；过时的 PR #38 待办没有覆盖当前 v.5 已上线状态。

### 当前阶段

**阶段：本地 `main` 已与 GitHub `main` 对齐；本轮记录提交并推送后再次核验工作区与远端提交。**

### 当前最高优先级

保持下方已验证路线：使用真实手机分别通过 Wi-Fi 与蜂窝网络打开 `https://wenqu.zhuofan.me/`，补齐物理终端双链路证据。
...
- 用户报告新功能文字在手机端溢出。修复保持“纸上书房”现有宣纸、墨色、朱砂、字体和间距体系，只补充浏览器文字缩放控制、可收缩宽度与长中英文断行；诊断建议、路线按钮、证据笔记标题、材料名和来源定位不再依赖设备默认断词行为。
- 侧栏遗留的 `v.4` 已同步为 `v.5`。证据笔记页边编号继续使用朱砂体系，但改用明暗主题独立的高对比文字色；计算对比度分别为 5.83:1 与 5.18:1。
...
- 本地 Production build 再次通过 1810 modules。1420px 下三张指标卡宽度均约 346px、末卡与网格右边距约 0；工具栏两列约 753/301px，两个辅助标签实际盒模型均为 1×1px，输入框与筛选框仍分别拥有可访问名称。390/360/320px 均为 `documentWidth = viewport`、可见元素越界 0；证据笔记面板 axe 违规 0、浏览器运行错误 0。PR [#44](https://github.com/lizhuofan-curry/wenqu/pull/44) 的 Web/API CI 与合并后 main CI 均通过，合并提交 `c18d36bbd3f9b2d7d9023007d28d1e2044b3bc23` 已精确发布为 Vercel Production 部署 `dpl_2VLMcgCj8MtUNDNcb646cXP1nvXA`；生产自定义域复测得到相同列宽、可访问名称和零越界结果。
- 用户第三轮截图显示特殊窄窗口中保持率摘要与证据笔记面板仍保留桌面横排。生产实时 HTML 已确认加载新版 `index-C8o25I03.css`，排除“旧 CSS 未上线”；代码薄弱点是应用外壳在 980px 已切换窄屏结构，但保持率到 720px、笔记工具栏到 480px 才改单列。现统一在 980px 内让保持率摘要、标题区、笔记导出与筛选区提前改单列；900px 内的三项指标使用两列时，第三项占满整行，650px 内继续单列。未改变宣纸、墨色、朱砂、字体、圆角或面板装饰。
- 本地 TypeScript 与 Vite Production build（1810 modules）通过；浏览器在 980/900/720/650/480/390/320px 七档测得 `documentWidth = viewport`、可见元素越界 0。980px 内保持率和笔记工具栏均为单列；900/720px 第三张指标卡占满整行，650px 内三张卡单列；浏览器运行错误 0。
- 完整质量门禁通过：保持率 8 个场景、诊断 35 项、证据笔记 50 项、两套 Ruff、Python 编译与 API 97 项测试均通过；仅保留 1 条既有 Starlette/httpx 第三方弃用警告。临时工作区首次 API 运行因 Windows 中文路径编码导致 3 项无法创建目录，切换到纯英文系统临时目录后完整通过，未当作代码失败。
- PR [#46](https://github.com/lizhuofan-curry/wenqu/pull/46) 的 Web/API CI 与合并后 main CI 均通过，合并提交 `7d95da3741938147b289ca8d5aaf81edb5fca5b4` 已从无 Git 元数据的精确快照发布到 Vercel Production。部署 `dpl_CG6DfxjQ1kSiMJuvxnfSyziyXsRE` 为 READY 并绑定 `https://wenqu.zhuofan.me`；生产资源更新为 `index-BfZ4nFxm.css`，健康接口继续返回 `status=ok, version=v.5`。生产七档宽度复测与本地结果一致，可见元素越界和浏览器错误均为 0。
...
- 每节提供“已理解 / 仍需复核”的显式自我标记；“仍需复核”进入当前学习会话内的去重清单。进入测验前若清单非空，会插入可逐项查看和移除的复核检查点，清单清空后才能开始测验。
...
- 已以普通 merge 将通过 GitHub CI 的 PR #36 提交 `5254f0a` 合入 PR #37 本地历史；自适应路线没有新增数据库迁移，002–004 仍保持独立发布闸门。
- 合并后完整 `pnpm check` 通过：诊断 UI **35 assertions passed**、保持率 **8 scenarios passed**、完整 API **97 passed**、Vite Production build **1807 modules**，两套 Ruff、Python 编译与 TypeScript 均通过，仅保留 1 条既有第三方弃用警告。PR #37 当前仍 **OPEN、未合并、未发布**，等待推送后的新 GitHub CI。

### 当前阶段

**阶段：PR #37 已在本地合入 #36 的生产安全历史并通过完整门禁；等待提交推送与新 GitHub CI，尚未合并或发布 Production。**

### 当前最高优先级

提交并推送 PR #37，等待新 CI；随后将同一已验证历史普通合入 PR #38。继续遵守 002 → 003 → 004 的独立生产迁移顺序；自适应路线本身不新增迁移。
