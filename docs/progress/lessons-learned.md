...
### 108. 组件响应式断点必须与应用外壳的窄屏切换保持一致

- 现象：生产站已经加载三列指标和 `sr-only` 修复，受控 320–390px 也没有页面横向滚动，但用户在特殊窄窗口中仍看到保持率摘要、空状态和笔记工具栏右侧被裁切。
- 根因：应用外壳在 980px 已进入无侧栏窄屏结构，保持率却到 720px、笔记工具栏到 480px 才改单列；上一轮只覆盖 1420px 桌面与 320–390px 手机极值，没有覆盖 720–980px 中间区间，也没有把生产资源版本核对与布局断点核对分开。
- 解决：先读取生产 HTML/CSS 哈希排除旧部署，再让保持率标题与摘要、笔记导出与筛选在 980px 内同步改单列；指标在两列状态下让第三项占满整行，650px 内继续单列。浏览器复测 980/900/720/650/480/390/320px 的文档宽度、组件计算列和可见元素边界。
- 预防：响应式验收除桌面和手机极值外，必须包含应用外壳断点及其两侧的中间宽度；每个新增面板要与壳层共享断点合同，并同时记录生产资源版本、文档宽度、内部网格列和元素边界，不能用单一 `scrollWidth` 结论替代。

## 2026-08-26｜本地主线与 GitHub main 对齐

- 本轮开始前已完整复读本文件；同步仅处理 Git 历史与项目记录，没有修改产品代码、生产配置或数据库，也没有输出任何密钥、令牌或完整连接地址。

### 109. 本地 main 长期分叉时应保留旧历史后重建跟踪分支

- 现象：本地 `main` 相对 `origin/main` 已领先 8、落后 80；直接拉取不能快进，强制重置或强推又会丢失或改写旧历史。同时当前功能分支还有两份未提交进展记录。
...
## 2026-08-27｜工程质量清理、排版收口与多材料专题

- 本轮开始前已完整复读本文件；改动限于 api/index.py 规范清理与 chunks 回填、前端 CSS token 化、本地专题功能与设计文档，没有修改数据库、没有执行迁移或部署，也没有输出任何密钥、令牌或完整连接地址。

### 111. CSS 自定义属性被引用但未定义时静默失效，需要对账引用集与定义集

- 现象：`pages.css` 引用 `--radius-soft`、`--paper-dark`，`study.css`/`shell.css`/`pages.css` 引用 `--shadow-soft`，但 `tokens.css` 从未定义它们；浏览器静默回退（圆角变 0、颜色/阴影无效），页面看起来"大致正常"，bug 长期无人发现。
- 根因：CSS 变量缺失不产生任何构建或运行时报错；多文件分散引用，没有机制保证引用与定义同步。
- 解决：提取全部 `var(--xxx)` 引用集合与 tokens.css 定义集合做差集对账，补齐三个缺失定义（含暗色主题变体）。
- 预防：新增/重命名 token 时跑一遍"引用-定义"差集检查；TSX 内联 style 注入的变量（如 `--progress`/`--score`）属合法运行时注入，对账时需排除以免误报。

### 112. 文档中的历史缺口描述会漂移，执行前必须实测复核

- 现象：文档记录"api/index.py 有 87 条历史 Ruff 项"，实测已降到 67 条；文档记录"上传材料冷启动读回管线未做"，实测读回主体已实现，仅剩 chunks 不回填的二阶缺口。
...
- 本轮开始前已完整复读本文件；审计覆盖应用代码、认证与所有权、上传/AI 路径、Supabase 检查、依赖与 GitHub 仓库设置。没有输出密钥、令牌或完整数据库连接地址，没有执行数据库迁移或生产部署。

### 114. 资源额度不能挂在可选 AI 分支里

- 现象：长度不超过 100 字、未配置 AI 或 AI 分支未进入的上传不会调用 `upload` 额度 RPC，攻击者可以反复上传短文件消耗解析、内存与数据库容量。
- 根因：额度检查被误当成 AI 调用计费，而不是上传资源准入；同时缺少账号级持久化材料总量上限。
- 解决：在读取和解析文件前统一执行账号材料容量检查与原子 `upload` 额度消费；持久化场景达到 50 份时拒绝，容量检查异常时返回 503 安全失败。
- 预防：每个可消耗 CPU、内存、外部调用或持久化空间的入口都要在最昂贵操作前独立限流；测试必须覆盖短输入、未配置供应商和检查失败路径。

### 115. JSON 语法合法不代表外部模型输出可以进入业务对象

- 现象：AI 返回列表、错误嵌套类型、超长字段或异常分值时，`json.loads()` 仍成功，后续代码才以 `.get()` 异常表现为 500。
- 根因：入口只验证 JSON 语法，没有验证顶层类型、字段白名单、数量、长度和数值边界。
- 解决：为上传生成结果建立 `extra=forbid` 的 Pydantic 模型，并限制章节/问题数量、文本长度与分值；校验失败由既有降级路径转为原文学习流。
- 预防：LLM、Webhook 和第三方 API 返回值都按不可信输入处理；在业务访问前完成结构、大小和枚举/数值边界校验，并保留畸形但语法合法的回归样本。

### 116. 漏洞审计工具本身也要在真实路径与 CI 环境中验证

- 现象：新增 `pip-audit` 后，本地项目虚拟环境因 Windows 中文路径下 `pip_api` 按 UTF-8 解码 `pip --version` 输出而失败；隔离环境能正常审计，并发现旧 `pytest 8.4.2` 的 `PYSEC-2026-1845`。
- 根因：工具运行兼容性与被审计依赖安全性是两个独立问题；只把命令写进 CI 不足以证明门禁可用，也不能把工具崩溃误报为“无漏洞”。
- 解决：将 pytest 升级到 9.1.1；本地使用隔离的 `pip-audit 2.10.1` 对两份清单复核为零已知漏洞，CI 在 Ubuntu 上继续直接运行固定版本工具。
- 预防：安全扫描必须记录工具版本、退出状态和真实结果；本地兼容性失败时换隔离环境交叉验证，但仍保留 CI 的独立执行，不把未执行当通过。
### 117. 创建分支失败后，分号串联命令仍可能继续提交到当前分支

- 现象：计划创建的 `codex/security-hardening` 已被历史工作占用，`git switch -c` 返回失败，但同一串命令中的暂存和提交仍继续执行，提交短暂落在本地 `main`。
- 根因：PowerShell 分号不会因前一条普通命令返回非零自动停止；同时创建前没有先核对本地与远端同名分支。
- 解决：先只读检查分支指向；把提交保存在新的日期分支 `codex/security-audit-fix-20260831`，切换后让本地 `main` 指回未改动的 `origin/main`，远端 main 从未被重写或推送。
- 预防：创建分支前先运行分支存在性检查；切换、暂存、提交不得用无退出检查的分号串联，必须在切换成功后重新读取当前分支名再提交。
## 2026-08-31｜安全修复生产发布

- 本轮按用户明确授权合并 PR #49、部署 Vercel Production 并验证线上接口；没有执行数据库迁移，没有输出密钥、令牌或完整数据库连接地址。

### 118. 精确归档失败必须阻断后续部署，不能只依赖最终构建失败

- 现象：本地尚未 fetch PR 合并提交时，`git archive a523a043...` 返回 `not a tree object`；同一 PowerShell 命令后续步骤仍创建 `.vercel/project.json` 并启动部署，产生一个 0 文件的失败构建。
- 根因：归档、复制项目绑定和部署使用分号串联，没有在归档后检查退出码，也没有在部署前断言关键文件与最小文件数量。Vercel 构建最终失败且未替换生产别名，但这是平台兜底，不是本地发布闸门有效。
- 解决：确认失败部署未切换别名；fetch 精确合并提交后验证对象类型为 commit，用独立归档文件解包，并检查 `package.json`、`api/index.py`、文件数 157、无 `.git` 后才单独启动 Production 部署。正确部署 READY 后再验收主域。
- 预防：发布脚本必须 fail-fast；fetch → cat-file → archive → extract → 关键文件/数量/Git 元数据 → deploy 每一步都检查退出状态。真正的部署命令单独执行，不能与快照生成放在无短路保护的命令串中。
